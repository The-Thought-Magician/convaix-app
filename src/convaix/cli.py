...
def list_cmd(db, source):
    ...
    store.close()
    if not rows:
        console.print("[yellow]No conversations found.[/yellow]"); return
    t = Table(title="Conversations")
    t.add_column("#", style="dim"); t.add_column("Source", style="magenta")
    t.add_column("Title", style="blue", ratio=2); t.add_column("Turns", style="green")
    t.add_column("Date", style="dim"); t.add_column("ID", style="dim")
...
@click.option("--db", default=DEFAULT_DB, envvar="CONVAIX_DB")
@click.option("--source", "-s"); @click.option("--limit", "-l", default=10, type=int)
...
def search(query, db, source, limit, mode):
    ...
    t.add_column("Sim", style="green", no_wrap=True); t.add_column("Src", style="magenta")
    t.add_column("Title", style="blue", max_width=30); t.add_column("Content", ratio=2)
    for r in results:
...
def validate(file_path):
    ...
    try:
        ...
        console.print(f"[red]Invalid[/red]: {e}"); raise SystemExit(1)
...
@click.option("--db", default=DEFAULT_DB, envvar="CONVAIX_DB")
@click.option("--source", "-s"); @click.option("--num-chunks", default=5, type=int)
def ask(question, db, source, num_chunks):
    ...
    if result["sources"]:
        ...
        t.add_column("Src", style="magenta"); t.add_column("Title", style="blue")
        t.add_column("Sim", style="green"); t.add_column("Type", style="dim")
...
@main.command()
@click.option("--db", default=DEFAULT_DB, envvar="CONVAIX_DB")
@click.option("--port", default=8000, type=int); @click.option("--host", default="127.0.0.1")
